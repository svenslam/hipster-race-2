...
interface WheelSelectorProps {
  onSpinComplete: (game: GameType) => void;
  isSpinningExternal: boolean;
}

export const WheelSelector: React.FC<WheelSelectorProps> = ({ onSpinComplete, isSpinningExternal }) => {
  ...
  const spin = () => {
    ...
    // Calculate rotation:
    // 360 * 5 (5 full spins) + specific angle to land on segment
    // Each segment is 90deg. 
    // Index 0 (F1): 0-90 (Target center 45) -> Rotation needs to align top pointer to this.
    // Actually, if pointer is at top (0deg), we need to rotate the wheel so the segment is at 0deg.
    // Segment 0 is at 0-90. To get center (45) to top (0/360), rotate -45.
    // Let's simplify: just add random full spins + rigid offset.
    ...
    // Notify parent immediately to block UI, but wait for animation to trigger callback
